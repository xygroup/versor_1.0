...
#define ITN for (int i = 0; i < mNum; ++i) {
#define END }
...
class Field : public CubicLattice {
 ...
 public:
  ...
  // MOVED TO vsr_draw
  //        void draw(float r = 1.0, float g = 1.0, float b = 1.0, float a
  //        = 1.0){
  ...
  //
  //
  //        void drawPush(float r = 1.0, float g = 1.0, float b = 1.0, float a
  //        = 1.0){
  //            for (int i = 0; i < mNum; ++i){
  //                GL::push();
  ...
  T diffYNbrs(int ix) const { return mData[nbr(ix).yt] - mData[nbr(ix).yb]; }
  ...
  void advect(const Field& prev, const Field<B>& f, double dt, bool ref) {
    ...
    BOUNDITER
    int tidx = idx(i, j, k);
    Pnt p = mPoint[tidx].trs(f.euler3d(mPoint[tidx]) *
                             -dt0);  // f[tidx] * -dt0 ); //Lattice Point
  ...
  Field& operator-=(const Field<MV<N2, IDX2, T2> >& f) {
    ITN mData[i] -= f[i];
    END return *this;
  }
...
template <>
void Field<Vec>::boundaryConditions(bool ref) {
  for (int i = 0; i < mFace.size(); ++i) {
    int ix = mFace[i];
    static Nbr n;
    n = mNbr[ix];
...
template <>
void Field<Vec>::init() {
  ITER mData[tidx] = Vec(mPoint[tidx]).unit();
  ITEND
}
...
//		double tdx;
//		tdx += mData[ nbr(idx).xr ].pos()[0] - mData [ nbr(idx).xl
//].pos()[0]; //xr - xl 		tdx += mData[ nbr(idx).yt ].pos()[1] - mData [
//nbr(idx).yb ].pos()[1]; //yt - yb 		tdx += mData[ nbr(idx).zb ].pos()[2] - mData
 ...
 public:
  Fluid(int _w, int _h, int _d)
      : mField(_w, _h, _d),
        mPrev(_w, _h, _d),
        mPressure(_w, _h, _d),
        mDivergence(_w, _h, _d),
        mDensity(_w * 2, _h * 2, _d * 2, .5),
        mDensityPrev(_w * 2, _h * 2, _d * 2, .5),
...
//
//				mData[ix] =  (ref) ?  Op::re( mData[ix+1],
//Dlp(0,0,1,0) ) : mData[ix+1]; 				mData[ix2] = (ref) ?  Op::re( mData[ix2-1],
//Dlp(0,0,1,0) ) : mData[ix2-1];
//
//			}
//		}
//		//x component
...
//				 ix2 = idx(i,mHeight-1,j);  ixo2 =
//idx(i,mHeight-2,j); 				mData[ix] = ref ? Op::re( mData[ixo], Dlp(0,1,0,0) ):
//mData[ixo];//pdata[ixo] * -1 : pdata[ixo]; // 				mData[ix2] = ref ?  Op::re(
//mData[ixo2], Dlp(0,1,0,0) ): mData[ixo2];//pdata[ixo2] * -1 : pdata[ixo2];//
////RIGHT
//			}
//		}
//		//y component
...
//				 ix2 = idx(mWidth-1,i,j);  ixo2 =
//idx(mWidth-2,i,j); 				mData[ix] = ref ?  Op::re( mData[ixo], Dlp(1,0,0,0) ) :
//mData[ixo];	//pdata[ixo] * -1 : pdata[ixo];//			//TOP
//				mData[ix2] = ref ? Op::re( mData[ixo2], Dlp(1,0,0,0) )
//: mData[ixo2];//pdata[ixo2] * -1 : pdata[ixo2];//			//BOTTOM
//			}
//		}
//
//
...
//		for ( i = 1; i < mWidth-1; ++i){
//			 ix = idx(i,0,0);
//			mData[ ix ] = ( mData[ mNbr[ ix ].yt ] +  mData[ mNbr[ ix
//].zb ] ) / 2.0;	//BOTTOM
//
//			 ix = idx(i,mHeight-1,0);
//			mData[ ix ] = ( mData[ mNbr[ ix ].yb ] +  mData[ mNbr[ ix
//].zb ]  ) / 2.0;   //TOP
//
...
//		for ( i = 1; i < mHeight-1; ++i){
//			 ix = idx(0,i,0);
//			mData[ ix ] = ( mData[ mNbr[ ix ].xr ] +  mData[ mNbr[ ix
//].zb ] ) / 2.0;
//
//			 ix = idx(mWidth-1,i,0);
//			mData[ ix ] = ( mData[ mNbr[ ix ].xl ] +  mData[ mNbr[ ix
//].zb ] ) / 2.0;
//
...
//		for ( i = 0; i < 8; ++i){
//			mData [ mCorner[i] ] =  (ref) ? sumNbrs( mCorner[i] ) / -1.0
//: sumNbrs( mCorner[i] ) / 3.0;
//		}
//
//
//	}
