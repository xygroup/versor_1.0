...
// Q: Given a Circle, can we find it's associated HopfBundle, and thereby a
// commuting Circle?
namespace vsr {

/// Feed in Spherical Coordinates of a 2-Sphere with One Pole Specified, get
/// 3-Sphere Fiber out
class HopfFiber {
  // Chirality
  bool bHanded;
 ...
 public:
  ...
  Cir fiber(const Vec& v) {
    Coord::Sph cs =
        Coord::vec2sph(v);  // spherical coordinates [-1,1] and [-.5,.5]
  ...
  Pnt pnt() { return Ro::pnt_cir(fiberA(), mPhase * PI); }
};
