...
inline Sca gp(const Drv& a, const Inf& b) { return Sca(); }
...
inline Inf_Par gp(const Drv& a, const Pnt& b) {
  return Inf_Par(-a[0] * b[3], -a[1] * b[3], -a[2] * b[3],
...
inline Mtd gp(const Drv& a, const Par& b) {
  return Mtd(a[0] * b[3] + a[1] * b[4] + a[2] * b[5], a[0] * b[4] - a[1] * b[3],
...
inline Par_Drb gp(const Drv& a, const Cir& b) {
  return Par_Drb(
      ...
      -a[0] * b[9] + a[1] * b[8] - a[2] * b[7], a[1] * b[0] + a[2] * b[1],
...
inline Inf_Cir gp(const Drv& a, const Sph& b) {
  return Inf_Cir(a[2] * b[3], -a[1] * b[3], a[0] * b[3],
                 a[1] * b[0] + a[2] * b[1], -a[0] * b[0] + a[2] * b[2],
...
inline Sca gp(const Drv& a, const Drv& b) { return Sca(); }

inline Sca gp(const Drv& a, const Drb& b) { return Sca(); }

inline Sca gp(const Drv& a, const Drt& b) { return Sca(); }
...
inline Drv_Lin gp(const Drv& a, const Lin& b) {
  return Drv_Lin(a[0] * b[3] + a[1] * b[4] + a[2] * b[5],
                 a[0] * b[4] - a[1] * b[3], a[0] * b[5] - a[2] * b[3],
                 a[1] * b[5] - a[2] * b[4]);
}
...
inline Drv gp(const Drv& a, const Flp& b) {
  return Drv(-a[0] * b[3], -a[1] * b[3], -a[2] * b[3]);
}

inline Drb gp(const Drv& a, const Dfp& b) {
  return Drb(-a[2] * b[3], a[1] * b[3], -a[0] * b[3]);
}
...
inline Par_Drb gp(const Drv& a, const Dal& b) {
  return Par_Drb(a[1] * b[0] + a[2] * b[1], -a[0] * b[0] + a[2] * b[2],
                 -a[0] * b[1] - a[1] * b[2],
...
inline Drv_Pln gp(const Drv& a, const Rot& b) {
  return Drv_Pln(a[0] * b[0] - a[1] * b[1] - a[2] * b[2],
...
inline Drv gp(const Drv& a, const Dil& b) {
  return Drv(a[0] * b[0] - a[0] * b[1], a[1] * b[0] - a[1] * b[1],
             a[2] * b[0] - a[2] * b[1]);
}

inline Drv_Dap gp(const Drv& a, const Trv& b) {
  return Drv_Dap(a[0] * b[1] + a[1] * b[2] + a[2] * b[3],
...
inline Drv_Pln gp(const Drv& a, const Mtd& b) {
  return Drv_Pln(a[0] * b[0] - a[0] * b[7] - a[1] * b[1] + a[1] * b[8] -
                     a[2] * b[2] + a[2] * b[9],
...
inline Par_Drb gp(const Drv& a, const Rtc& b) {
  return Par_Drb(-a[0] * b[3] + a[1] * b[5] + a[2] * b[6],
...
inline Mtd gp(const Drv& a, const Mtt& b) {
  return Mtd(a[0] * b[4] + a[1] * b[5] + a[2] * b[6],
...
inline Drv_Pln gp(const Drv& a, const Rtd& b) {
  return Drv_Pln(a[0] * b[0] - a[0] * b[4] - a[1] * b[1] + a[1] * b[5] -
                     a[2] * b[2] + a[2] * b[6],
...
inline Drv gp(const Drv& a, const Tsd& b) {
  return Drv(a[0] * b[0] - a[0] * b[4], a[1] * b[0] - a[1] * b[4],
             a[2] * b[0] - a[2] * b[4]);
}

inline Mtd gp(const Drv& a, const Trt& b) {
  return Mtd(a[0] * b[4] + a[1] * b[5] + a[2] * b[6], a[0] * b[5] - a[1] * b[4],
             ...
             a[0] * b[2] - a[0] * b[12] + a[1] * b[3] - a[1] * b[13] +
                 a[2] * b[0] - a[2] * b[10],
             a[0] * b[4] + a[1] * b[5] + a[2] * b[6], a[0] * b[5] - a[1] * b[4],
             a[0] * b[6] - a[2] * b[4], a[1] * b[6] - a[2] * b[5],
             a[0] * b[3] - a[0] * b[13] - a[1] * b[2] + a[1] * b[12] +
                 a[2] * b[1] - a[2] * b[11]);
}
...
inline Sca op(const Drv& a, const Inf& b) { return Sca(); }

inline Sca op(const Drv& a, const Mnk& b) { return Sca(); }

inline Mnv op(const Drv& a, const Hyp& b) {
  return Mnv(-a[0] * b[0], -a[1] * b[0], -a[2] * b[0]);
}

inline Sca op(const Drv& a, const Pss& b) { return Sca(); }
...
inline Sca op(const Drv& a, const Sph& b) { return Sca(); }
...
inline Sca op(const Drv& a, const Drv& b) { return Sca(); }

inline Sca op(const Drv& a, const Drb& b) { return Sca(); }

inline Sca op(const Drv& a, const Drt& b) { return Sca(); }
...
inline Sca op(const Drv& a, const Tnt& b) { return Sca(); }

inline Sca op(const Drv& a, const Lin& b) { return Sca(); }

inline Sca op(const Drv& a, const Pln& b) { return Sca(); }

inline Sca op(const Drv& a, const Flp& b) { return Sca(); }

inline Sca op(const Drv& a, const Dfp& b) { return Sca(); }
...
inline Sca op(const Drv& a, const Tri& b) { return Sca(); }
...
inline Sca op(const Drv& a, const Daf& b) { return Sca(); }

inline Pss op(const Drv& a, const Dal& b) {
  return Pss(-a[0] * b[2] + a[1] * b[1] - a[2] * b[0]);
}

inline Sca op(const Drv& a, const Mnv& b) { return Sca(); }

inline Sca ip(const Drv& a, const Sca& b) { return Sca(); }

inline Sca ip(const Drv& a, const Ori& b) { return Sca(); }

inline Sca ip(const Drv& a, const Inf& b) { return Sca(); }

inline Sca ip(const Drv& a, const Mnk& b) { return Sca(); }

inline Sca ip(const Drv& a, const Hyp& b) { return Sca(); }

inline Drb ip(const Drv& a, const Pss& b) {
  return Drb(a[2] * b[0], -a[1] * b[0], a[0] * b[0]);
}

inline Sca ip(const Drv& a, const Pnt& b) { return Sca(); }
...
inline Sca ip(const Drv& a, const Drv& b) { return Sca(); }

inline Sca ip(const Drv& a, const Drb& b) { return Sca(); }

inline Sca ip(const Drv& a, const Drt& b) { return Sca(); }
...
inline Inf ip(const Drv& a, const Lin& b) {
  return Inf(a[0] * b[3] + a[1] * b[4] + a[2] * b[5]);
}
...
inline Sca ip(const Drv& a, const Flp& b) { return Sca(); }

inline Sca ip(const Drv& a, const Dfp& b) { return Sca(); }

inline Sca ip(const Drv& a, const Dll& b) { return Sca(); }

inline Sca ip(const Drv& a, const Dlp& b) { return Sca(); }

inline Sca ip(const Drv& a, const Vec& b) { return Sca(); }

inline Sca ip(const Drv& a, const Biv& b) { return Sca(); }

inline Sca ip(const Drv& a, const Tri& b) { return Sca(); }

inline Sca ip(const Drv& a, const Aff& b) { return Sca(); }
...
inline Drv sp(const Drv& drv, const Rot& rot) {
  const Rot& t1 = rot;
  const Drv& t2 = drv;
  Drv_Pln t3(t1[0] * t2[0] + t1[1] * t2[1] + t1[2] * t2[2],
             t1[0] * t2[1] - t1[1] * t2[0] + t1[3] * t2[2],
             t1[0] * t2[2] - t1[2] * t2[0] - t1[3] * t2[1],
             t1[1] * t2[2] - t1[2] * t2[1] + t1[3] * t2[0]);
  const Rot& t4 = rot;
  Rot t5 = reverse(t4);
  return Drv(t3[0] * t5[0] - t3[1] * t5[1] - t3[2] * t5[2] - t3[3] * t5[3],
...
inline Drv sp(const Drv& drv, const Dil& dil) {
  const Dil& t1 = dil;
  const Drv& t2 = drv;
  Drv t3(t1[0] * t2[0] + t1[1] * t2[0], t1[0] * t2[1] + t1[1] * t2[1],
         t1[0] * t2[2] + t1[1] * t2[2]);
  const Dil& t4 = dil;
  Dil t5 = reverse(t4);
  return Drv(t3[0] * t5[0] - t3[0] * t5[1], t3[1] * t5[0] - t3[1] * t5[1],
             t3[2] * t5[0] - t3[2] * t5[1]);
}
...
inline Drv sp(const Drv& drv, const Trv& trv) {
  const Trv& t1 = trv;
  const Drv& t2 = drv;
  Drv_Dap t3(t1[1] * t2[0] + t1[2] * t2[1] + t1[3] * t2[2],
  ...
  const Trv& t4 = trv;
  Trv t5 = reverse(t4);
  return Drv(t3[4] * t5[0], t3[5] * t5[0], t3[6] * t5[0]);
}
...
inline Drv sp(const Drv& drv, const Mtd& mtd) {
  const Mtd& t1 = mtd;
  const Drv& t2 = drv;
  Drv_Pln t3(t1[0] * t2[0] + t1[1] * t2[1] + t1[2] * t2[2] + t1[7] * t2[0] +
                 t1[8] * t2[1] + t1[9] * t2[2],
  ...
  const Mtd& t4 = mtd;
  Mtd t5 = reverse(t4);
  return Drv(t3[0] * t5[0] - t3[0] * t5[7] - t3[1] * t5[1] + t3[1] * t5[8] -
...
inline Drv sp(const Drv& drv, const Trs& trs) {
  const Trs& t1 = trs;
  const Drv& t2 = drv;
  Drv t3(t1[0] * t2[0], t1[0] * t2[1], t1[0] * t2[2]);
  const Trs& t4 = trs;
  Trs t5 = reverse(t4);
  return Drv(t3[0] * t5[0], t3[1] * t5[0], t3[2] * t5[0]);
}
...
inline Drv sp(const Drv& drv, const Rtc& rtc) {
  const Rtc& t1 = rtc;
  const Drv& t2 = drv;
  Par_Drb t3(t1[3] * t2[0] + t1[5] * t2[1] + t1[6] * t2[2],
  ...
  Rtc t5 = reverse(t4);
  return Drv(
      t3[0] * t5[4] - t3[1] * t5[8] - t3[2] * t5[9] - t3[3] * t5[0] +
          t3[3] * t5[11] - t3[4] * t5[1] + t3[4] * t5[12] - t3[5] * t5[2] +
          t3[5] * t5[13] + t3[6] * t5[14] - t3[6] * t5[15] + t3[7] * t5[4] -
          t3[8] * t5[8] - t3[9] * t5[9] - t3[10] * t5[10] - t3[11] * t5[10],
      t3[0] * t5[8] + t3[1] * t5[4] - t3[2] * t5[10] - t3[3] * t5[1] +
          t3[3] * t5[12] + t3[4] * t5[0] - t3[4] * t5[11] - t3[5] * t5[14] +
          t3[5] * t5[15] - t3[6] * t5[2] + t3[6] * t5[13] + t3[7] * t5[8] +
          t3[8] * t5[4] - t3[9] * t5[10] + t3[10] * t5[9] + t3[11] * t5[9],
      t3[0] * t5[9] + t3[1] * t5[10] + t3[2] * t5[4] - t3[3] * t5[2] +
          t3[3] * t5[13] + t3[4] * t5[14] - t3[4] * t5[15] + t3[5] * t5[0] -
          t3[5] * t5[11] + t3[6] * t5[1] - t3[6] * t5[12] + t3[7] * t5[9] +
          t3[8] * t5[10] + t3[9] * t5[4] - t3[10] * t5[8] - t3[11] * t5[8]);
}
...
inline Drv sp(const Drv& drv, const Mtt& mtt) {
  const Mtt& t1 = mtt;
  const Drv& t2 = drv;
  Mtd t3(t1[4] * t2[0] + t1[5] * t2[1] + t1[6] * t2[2],
  ...
  return Drv(
      t3[0] * t5[7] + t3[1] * t5[8] + t3[2] * t5[9] - t3[3] * t5[15] +
          ...
          t3[6] * t5[2] + t3[6] * t5[12] + t3[7] * t5[7] + t3[8] * t5[8] +
          t3[9] * t5[9] - t3[10] * t5[15] - t3[11] * t5[3] + t3[11] * t5[13],
      t3[0] * t5[8] - t3[1] * t5[7] + t3[2] * t5[15] + t3[3] * t5[9] +
          t3[4] * t5[1] - t3[4] * t5[11] + t3[5] * t5[0] - t3[5] * t5[10] -
          t3[6] * t5[3] + t3[6] * t5[13] + t3[7] * t5[8] - t3[8] * t5[7] +
          t3[9] * t5[15] + t3[10] * t5[9] + t3[11] * t5[2] - t3[11] * t5[12],
      t3[0] * t5[9] - t3[1] * t5[15] - t3[2] * t5[7] - t3[3] * t5[8] +
          t3[4] * t5[2] - t3[4] * t5[12] + t3[5] * t5[3] - t3[5] * t5[13] +
          t3[6] * t5[0] - t3[6] * t5[10] + t3[7] * t5[9] - t3[8] * t5[15] -
          t3[9] * t5[7] - t3[10] * t5[8] - t3[11] * t5[1] + t3[11] * t5[11]);
}
...
inline Drv sp(const Drv& drv, const Rtt& rtt) {
  ...
  const Rtt& t4 = rtt;
  Rtt t5 = reverse(t4);
  return Drv(t3[4] * t5[0] - t3[5] * t5[1] - t3[6] * t5[2] - t3[11] * t5[3],
...
inline Drv sp(const Drv& drv, const Rtd& rtd) {
  const Rtd& t1 = rtd;
  const Drv& t2 = drv;
  Drv_Pln t3(t1[0] * t2[0] + t1[1] * t2[1] + t1[2] * t2[2] + t1[4] * t2[0] +
                 t1[5] * t2[1] + t1[6] * t2[2],
  ...
  const Rtd& t4 = rtd;
  Rtd t5 = reverse(t4);
  return Drv(t3[0] * t5[0] - t3[0] * t5[4] - t3[1] * t5[1] + t3[1] * t5[5] -
...
inline Drv sp(const Drv& drv, const Tvd& tvd) {
  ...
  const Tvd& t4 = tvd;
  Tvd t5 = reverse(t4);
  return Drv(t3[4] * t5[0] - t3[4] * t5[4], t3[5] * t5[0] - t3[5] * t5[4],
             t3[6] * t5[0] - t3[6] * t5[4]);
}
...
inline Drv sp(const Drv& drv, const Tsd& tsd) {
  const Tsd& t1 = tsd;
  const Drv& t2 = drv;
  Drv t3(t1[0] * t2[0] + t1[4] * t2[0], t1[0] * t2[1] + t1[4] * t2[1],
         t1[0] * t2[2] + t1[4] * t2[2]);
  const Tsd& t4 = tsd;
  Tsd t5 = reverse(t4);
  return Drv(t3[0] * t5[0] - t3[0] * t5[4], t3[1] * t5[0] - t3[1] * t5[4],
             t3[2] * t5[0] - t3[2] * t5[4]);
}
...
inline Drv sp(const Drv& drv, const Trt& trt) {
  ...
  return Drv(
      t3[0] * t5[7] + t3[1] * t5[8] + t3[2] * t5[9] + t3[4] * t5[0] -
          ...
          t3[6] * t5[12] + t3[7] * t5[7] + t3[8] * t5[8] + t3[9] * t5[9] -
          t3[11] * t5[3] + t3[11] * t5[13],
      t3[0] * t5[8] - t3[1] * t5[7] + t3[3] * t5[9] + t3[4] * t5[1] -
          t3[4] * t5[11] + t3[5] * t5[0] - t3[5] * t5[10] - t3[6] * t5[3] +
          t3[6] * t5[13] + t3[7] * t5[8] - t3[8] * t5[7] + t3[10] * t5[9] +
          t3[11] * t5[2] - t3[11] * t5[12],
      t3[0] * t5[9] - t3[2] * t5[7] - t3[3] * t5[8] + t3[4] * t5[2] -
          t3[4] * t5[12] + t3[5] * t5[3] - t3[5] * t5[13] + t3[6] * t5[0] -
          t3[6] * t5[10] + t3[7] * t5[9] - t3[9] * t5[7] - t3[10] * t5[8] -
...
inline Drv sp(const Drv& drv, const Tvt& tvt) {
  ...
  return Drv(
      t3[0] * t5[7] + t3[1] * t5[8] + t3[2] * t5[9] - t3[3] * t5[14] +
          t3[4] * t5[0] - t3[4] * t5[10] - t3[5] * t5[1] + t3[5] * t5[11] -
          t3[6] * t5[2] + t3[6] * t5[12] + t3[7] * t5[7] + t3[8] * t5[8] +
          t3[9] * t5[9] - t3[10] * t5[14] - t3[11] * t5[3] + t3[11] * t5[13],
      t3[0] * t5[8] - t3[1] * t5[7] + t3[2] * t5[14] + t3[3] * t5[9] +
          t3[4] * t5[1] - t3[4] * t5[11] + t3[5] * t5[0] - t3[5] * t5[10] -
          t3[6] * t5[3] + t3[6] * t5[13] + t3[7] * t5[8] - t3[8] * t5[7] +
          t3[9] * t5[14] + t3[10] * t5[9] + t3[11] * t5[2] - t3[11] * t5[12],
      t3[0] * t5[9] - t3[1] * t5[14] - t3[2] * t5[7] - t3[3] * t5[8] +
          t3[4] * t5[2] - t3[4] * t5[12] + t3[5] * t5[3] - t3[5] * t5[13] +
          t3[6] * t5[0] - t3[6] * t5[10] + t3[7] * t5[9] - t3[8] * t5[14] -
          t3[9] * t5[7] - t3[10] * t5[8] - t3[11] * t5[1] + t3[11] * t5[11]);
}
...
inline Drv sp(const Drv& drv, const Bst& bst) {
  ...
  return Drv(
      t3[0] * t5[7] + t3[1] * t5[8] + t3[2] * t5[9] + t3[4] * t5[0] -
          t3[4] * t5[10] - t3[5] * t5[1] - t3[6] * t5[2] + t3[7] * t5[7] +
          t3[8] * t5[8] + t3[9] * t5[9] - t3[11] * t5[3],
      t3[0] * t5[8] - t3[1] * t5[7] + t3[3] * t5[9] + t3[4] * t5[1] +
          t3[5] * t5[0] - t3[5] * t5[10] - t3[6] * t5[3] + t3[7] * t5[8] -
          t3[8] * t5[7] + t3[10] * t5[9] + t3[11] * t5[2],
      t3[0] * t5[9] - t3[2] * t5[7] - t3[3] * t5[8] + t3[4] * t5[2] +
          t3[5] * t5[3] + t3[6] * t5[0] - t3[6] * t5[10] + t3[7] * t5[9] -
          t3[9] * t5[7] - t3[10] * t5[8] - t3[11] * t5[1]);
}
...
inline Drv re(const Drv& drv, const Vec& vec) {
  ...
  const Vec& t5 = vec;
  Vec t6 = reverse(t5);
  return Drv(-t4[0] * t6[0] - t4[1] * t6[1] - t4[2] * t6[2],
...
inline Drv re(const Drv& drv, const Pln& pln) {
  ...
  const Drv& t2 = drv;
  Drv t3 = involute(t2);
  Drv_Pln t4(t1[0] * t3[1] + t1[1] * t3[2], -t1[0] * t3[0] + t1[2] * t3[2],
             -t1[1] * t3[0] - t1[2] * t3[1],
             t1[0] * t3[2] - t1[1] * t3[1] + t1[2] * t3[0]);
  const Pln& t5 = pln;
  Pln t6 = reverse(t5);
  return Drv(t4[1] * t6[0] + t4[2] * t6[1] + t4[3] * t6[2],
...
inline Drv re(const Drv& drv, const Lin& lin) {
  ...
  const Drv& t2 = drv;
  Drv t3 = involute(t2);
  Drv_Lin t4(t1[3] * t3[0] + t1[4] * t3[1] + t1[5] * t3[2],
             t1[3] * t3[1] - t1[4] * t3[0], t1[3] * t3[2] - t1[5] * t3[0],
             t1[4] * t3[2] - t1[5] * t3[1]);
  const Lin& t5 = lin;
  Lin t6 = reverse(t5);
  return Drv(t4[0] * t6[3] + t4[1] * t6[4] + t4[2] * t6[5],
...
inline Drv re(const Drv& drv, const Pnt& pnt) {
  ...
  const Drv& t2 = drv;
  Drv t3 = involute(t2);
  Inf_Par t4(t1[3] * t3[0], t1[3] * t3[1], t1[3] * t3[2],
  ...
  const Pnt& t5 = pnt;
  Pnt t6 = reverse(t5);
  return Drv(t4[0] * t6[4] - t4[3] * t6[0] - t4[4] * t6[1] - t4[5] * t6[2] +
                 t4[7] * t6[4],
             t4[1] * t6[4] - t4[3] * t6[1] + t4[4] * t6[0] - t4[6] * t6[2] +
                 t4[8] * t6[4],
             t4[2] * t6[4] - t4[3] * t6[2] + t4[5] * t6[0] + t4[6] * t6[1] +
                 t4[9] * t6[4]);
}
...
inline Drv re(const Drv& drv, const Par& par) {
  ...
  const Drv& t2 = drv;
  Drv t3 = involute(t2);
  Mtd t4(t1[3] * t3[0] + t1[4] * t3[1] + t1[5] * t3[2],
         t1[3] * t3[1] - t1[4] * t3[0], t1[3] * t3[2] - t1[5] * t3[0],
         t1[4] * t3[2] - t1[5] * t3[1],
  ...
  const Par& t5 = par;
  Par t6 = reverse(t5);
  return Drv(t4[0] * t6[6] + t4[1] * t6[7] + t4[2] * t6[8] - t4[4] * t6[9] -
                 t4[5] * t6[0] - t4[6] * t6[1] + t4[7] * t6[6] + t4[8] * t6[7] +
                 t4[9] * t6[8] - t4[11] * t6[2],
             t4[0] * t6[7] - t4[1] * t6[6] + t4[3] * t6[8] + t4[4] * t6[0] -
                 t4[5] * t6[9] - t4[6] * t6[2] + t4[7] * t6[7] - t4[8] * t6[6] +
                 t4[10] * t6[8] + t4[11] * t6[1],
             t4[0] * t6[8] - t4[2] * t6[6] - t4[3] * t6[7] + t4[4] * t6[1] +
                 t4[5] * t6[2] - t4[6] * t6[9] + t4[7] * t6[8] - t4[9] * t6[6] -
                 t4[10] * t6[7] - t4[11] * t6[0]);
}
...
inline Drv re(const Drv& drv, const Cir& cir) {
  ...
  const Drv& t2 = drv;
  Drv t3 = involute(t2);
  Par_Drb t4(t1[0] * t3[1] + t1[1] * t3[2], -t1[0] * t3[0] + t1[2] * t3[2],
             -t1[1] * t3[0] - t1[2] * t3[1],
  ...
  const Cir& t5 = cir;
  Cir t6 = reverse(t5);
  return Drv(-t4[1] * t6[3] - t4[2] * t6[4] + t4[3] * t6[6] + t4[4] * t6[7] +
                 t4[5] * t6[8] + t4[6] * t6[9] - t4[8] * t6[3] - t4[9] * t6[4] -
                 t4[10] * t6[5] - t4[11] * t6[5],
             t4[0] * t6[3] - t4[2] * t6[5] + t4[3] * t6[7] - t4[4] * t6[6] -
                 t4[5] * t6[9] + t4[6] * t6[8] + t4[7] * t6[3] - t4[9] * t6[5] +
                 t4[10] * t6[4] + t4[11] * t6[4],
             t4[0] * t6[4] + t4[1] * t6[5] + t4[3] * t6[8] + t4[4] * t6[9] -
                 t4[5] * t6[6] - t4[6] * t6[7] + t4[7] * t6[4] + t4[8] * t6[5] -
                 t4[10] * t6[3] - t4[11] * t6[3]);
}
...
inline Drv re(const Drv& drv, const Sph& sph) {
  ...
  const Drv& t2 = drv;
  Drv t3 = involute(t2);
  Inf_Cir t4(t1[3] * t3[2], -t1[3] * t3[1], t1[3] * t3[0],
             t1[0] * t3[1] + t1[1] * t3[2], -t1[0] * t3[0] + t1[2] * t3[2],
             -t1[1] * t3[0] - t1[2] * t3[1], -t1[3] * t3[2], t1[3] * t3[1],
             -t1[3] * t3[0], t1[0] * t3[2] - t1[1] * t3[1] + t1[2] * t3[0]);
  const Sph& t5 = sph;
  Sph t6 = reverse(t5);
  return Drv(-t4[2] * t6[4] + t4[4] * t6[0] + t4[5] * t6[1] - t4[8] * t6[4] +
                 t4[9] * t6[2],
