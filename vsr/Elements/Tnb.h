...
inline Sca gp(const Tnb& a, const Ori& b) { return Sca(); }
...
inline Ori_Drb gp(const Tnb& a, const Hyp& b) {
  return Ori_Drb(-a[0] * b[1], -a[1] * b[1], -a[2] * b[1], a[0] * b[1],
                 a[1] * b[1], a[2] * b[1]);
}
...
inline Ori_Cir gp(const Tnb& a, const Pnt& b) {
  return Ori_Cir(-a[0] * b[4], -a[1] * b[4], -a[2] * b[4],
                 -a[0] * b[1] - a[1] * b[2], a[0] * b[0] - a[2] * b[2],
                 a[1] * b[0] + a[2] * b[1], a[0] * b[4], a[1] * b[4],
                 a[2] * b[4], -a[0] * b[2] + a[1] * b[1] - a[2] * b[0]);
}

inline Par_Tnb gp(const Tnb& a, const Par& b) {
  return Par_Tnb(
      a[0] * b[7] + a[1] * b[8], -a[0] * b[6] + a[2] * b[8],
      -a[1] * b[6] - a[2] * b[7], -a[0] * b[0] - a[1] * b[1] - a[2] * b[2],
      a[0] * b[9] - a[1] * b[2] + a[2] * b[1],
      a[0] * b[2] + a[1] * b[9] - a[2] * b[0],
      -a[0] * b[1] + a[1] * b[0] + a[2] * b[9], -a[0] * b[7] - a[1] * b[8],
...
inline Rvd gp(const Tnb& a, const Cir& b) {
  return Rvd(a[0] * b[3] + a[1] * b[4] + a[2] * b[5], a[1] * b[5] - a[2] * b[4],
             -a[0] * b[5] + a[2] * b[3], a[0] * b[4] - a[1] * b[3],
...
inline Ori_Par gp(const Tnb& a, const Sph& b) {
  return Ori_Par(-a[2] * b[4], a[1] * b[4], -a[0] * b[4],
                 -a[0] * b[0] - a[1] * b[1] - a[2] * b[2],
                 -a[1] * b[2] + a[2] * b[1], a[0] * b[2] - a[2] * b[0],
                 -a[0] * b[1] + a[1] * b[0], a[2] * b[4], -a[1] * b[4],
                 a[0] * b[4]);
}

inline Par_Tnb gp(const Tnb& a, const Sta& b) {
  return Par_Tnb(a[0] * b[7] + a[1] * b[8], -a[0] * b[6] + a[2] * b[8],
                 -a[1] * b[6] - a[2] * b[7],
                 -a[0] * b[0] - a[1] * b[1] - a[2] * b[2],
                 -a[1] * b[2] + a[2] * b[1], a[0] * b[2] - a[2] * b[0],
                 -a[0] * b[1] + a[1] * b[0], -a[0] * b[7] - a[1] * b[8],
                 a[0] * b[6] - a[2] * b[8], a[1] * b[6] + a[2] * b[7],
                 a[0] * b[8] - a[1] * b[7] + a[2] * b[6],
                 -a[0] * b[8] + a[1] * b[7] - a[2] * b[6]);
}
...
inline Sca gp(const Tnb& a, const Tnv& b) { return Sca(); }

inline Sca gp(const Tnb& a, const Tnb& b) { return Sca(); }

inline Sca gp(const Tnb& a, const Tnt& b) { return Sca(); }
...
inline Rvd gp(const Tnb& a, const Lin& b) {
  return Rvd(
      ...
      -a[1] * b[2] + a[2] * b[1], a[0] * b[2] - a[2] * b[0],
      -a[0] * b[1] + a[1] * b[0], -a[0] * b[5] + a[1] * b[4] - a[2] * b[3]);
}

inline Ori_Par gp(const Tnb& a, const Pln& b) {
  return Ori_Par(-a[2] * b[3], a[1] * b[3], -a[0] * b[3],
                 -a[0] * b[0] - a[1] * b[1] - a[2] * b[2],
                 -a[1] * b[2] + a[2] * b[1], a[0] * b[2] - a[2] * b[0],
...
inline Par_Tnb gp(const Tnb& a, const Dll& b) {
  return Par_Tnb(a[0] * b[4] + a[1] * b[5], -a[0] * b[3] + a[2] * b[5],
                 -a[1] * b[3] - a[2] * b[4],
                 -a[0] * b[0] - a[1] * b[1] - a[2] * b[2],
                 -a[1] * b[2] + a[2] * b[1], a[0] * b[2] - a[2] * b[0],
...
inline Ori_Cir gp(const Tnb& a, const Dlp& b) {
  return Ori_Cir(-a[0] * b[3], -a[1] * b[3], -a[2] * b[3],
                 -a[0] * b[1] - a[1] * b[2], a[0] * b[0] - a[2] * b[2],
                 a[1] * b[0] + a[2] * b[1], a[0] * b[3], a[1] * b[3],
...
inline Tnv gp(const Tnb& a, const Afp& b) {
  return Tnv(a[2] * b[3], -a[1] * b[3], a[0] * b[3]);
}

inline Tnb gp(const Tnb& a, const Dap& b) {
  return Tnb(a[0] * b[3], a[1] * b[3], a[2] * b[3]);
}
...
inline Tnv_Biv gp(const Tnb& a, const Dal& b) {
  return Tnv_Biv(-a[0] * b[4] - a[1] * b[5], a[0] * b[3] - a[2] * b[5],
                 a[1] * b[3] + a[2] * b[4],
                 -a[0] * b[5] + a[1] * b[4] - a[2] * b[3]);
}
...
inline Tnv_Vec gp(const Tnb& a, const Rot& b) {
  return Tnv_Vec(-a[0] * b[1] - a[1] * b[2] - a[2] * b[3],
...
inline Par_Tnb gp(const Tnb& a, const Mot& b) {
  return Par_Tnb(a[0] * b[5] + a[1] * b[6] - a[2] * b[7],
...
inline Tnb gp(const Tnb& a, const Dil& b) {
  return Tnb(a[0] * b[0] + a[0] * b[1], a[1] * b[0] + a[1] * b[1],
             a[2] * b[0] + a[2] * b[1]);
}
...
inline Par_Tnb gp(const Tnb& a, const Mtd& b) {
  return Par_Tnb(a[0] * b[5] + a[1] * b[6] - a[2] * b[11],
...
inline Tnv_Dfp gp(const Tnb& a, const Trs& b) {
  return Tnv_Dfp(a[0] * b[2] + a[1] * b[3], -a[0] * b[1] + a[2] * b[3],
...
inline Rvd gp(const Tnb& a, const Rtc& b) {
  return Rvd(a[0] * b[8] + a[1] * b[9] + a[2] * b[10],
...
inline Par_Tnb gp(const Tnb& a, const Mtt& b) {
  return Par_Tnb(a[0] * b[8] + a[1] * b[9] - a[2] * b[15],
...
inline Tnv_Vec gp(const Tnb& a, const Rtd& b) {
  return Tnv_Vec(-a[0] * b[1] - a[0] * b[5] - a[1] * b[2] - a[1] * b[6] -
                     a[2] * b[3] - a[2] * b[7],
...
inline Tnb gp(const Tnb& a, const Tvd& b) {
  return Tnb(a[0] * b[0] + a[0] * b[4], a[1] * b[0] + a[1] * b[4],
             a[2] * b[0] + a[2] * b[4]);
}
...
inline Par_Tnb gp(const Tnb& a, const Trt& b) {
  return Par_Tnb(a[0] * b[8] + a[1] * b[9], -a[0] * b[7] + a[2] * b[9],
                 -a[1] * b[7] - a[2] * b[8],
                 -a[0] * b[1] - a[0] * b[11] - a[1] * b[2] - a[1] * b[12] -
                     a[2] * b[3] - a[2] * b[13],
                 a[0] * b[0] + a[0] * b[10] - a[1] * b[3] - a[1] * b[13] +
...
inline Tnv_Vec gp(const Tnb& a, const Rvd& b) {
  return Tnv_Vec(-a[0] * b[1] - a[0] * b[8] - a[1] * b[2] - a[1] * b[9] -
                     a[2] * b[3] - a[2] * b[10],
                 a[0] * b[0] + a[0] * b[7] - a[1] * b[3] - a[1] * b[10] +
                     a[2] * b[2] + a[2] * b[9],
                 a[0] * b[3] + a[0] * b[10] + a[1] * b[0] + a[1] * b[7] -
...
inline Par_Tnb gp(const Tnb& a, const Tvt& b) {
  return Par_Tnb(a[0] * b[8] + a[1] * b[9] - a[2] * b[14],
...
inline Par_Tnb gp(const Tnb& a, const Bst& b) {
  return Par_Tnb(
      ...
      -a[0] * b[8] - a[1] * b[9], a[0] * b[7] - a[2] * b[9],
      a[1] * b[7] + a[2] * b[8], a[0] * b[9] - a[1] * b[8] + a[2] * b[7],
      -a[0] * b[9] + a[1] * b[8] - a[2] * b[7]);
}
...
inline Sca op(const Tnb& a, const Cir& b) { return Sca(); }

inline Sca op(const Tnb& a, const Sph& b) { return Sca(); }
...
inline Sca op(const Tnb& a, const Drb& b) { return Sca(); }

inline Sca op(const Tnb& a, const Drt& b) { return Sca(); }

inline Sca op(const Tnb& a, const Tnv& b) { return Sca(); }

inline Sca op(const Tnb& a, const Tnb& b) { return Sca(); }

inline Sca op(const Tnb& a, const Tnt& b) { return Sca(); }

inline Sca op(const Tnb& a, const Lin& b) { return Sca(); }

inline Sca op(const Tnb& a, const Pln& b) { return Sca(); }

inline Pss op(const Tnb& a, const Flp& b) {
  return Pss(-a[0] * b[2] + a[1] * b[1] - a[2] * b[0]);
}

inline Sca op(const Tnb& a, const Dfp& b) { return Sca(); }
...
inline Sca op(const Tnb& a, const Biv& b) { return Sca(); }

inline Sca op(const Tnb& a, const Tri& b) { return Sca(); }

inline Tnt op(const Tnb& a, const Aff& b) {
  return Tnt(-a[0] * b[2] + a[1] * b[1] - a[2] * b[0]);
}

inline Sca op(const Tnb& a, const Afl& b) { return Sca(); }

inline Sca op(const Tnb& a, const Afp& b) { return Sca(); }

inline Sca op(const Tnb& a, const Dap& b) { return Sca(); }

inline Sca op(const Tnb& a, const Daf& b) { return Sca(); }

inline Sca op(const Tnb& a, const Dal& b) { return Sca(); }

inline Sca op(const Tnb& a, const Mnv& b) { return Sca(); }

inline Sca ip(const Tnb& a, const Sca& b) { return Sca(); }

inline Sca ip(const Tnb& a, const Ori& b) { return Sca(); }

inline Sca ip(const Tnb& a, const Inf& b) { return Sca(); }

inline Sca ip(const Tnb& a, const Mnk& b) { return Sca(); }

inline Sca ip(const Tnb& a, const Hyp& b) { return Sca(); }

inline Tnv ip(const Tnb& a, const Pss& b) {
  return Tnv(a[2] * b[0], -a[1] * b[0], a[0] * b[0]);
}

inline Sca ip(const Tnb& a, const Pnt& b) { return Sca(); }

inline Sca ip(const Tnb& a, const Par& b) { return Sca(); }
...
inline Aff ip(const Tnb& a, const Sph& b) {
  return Aff(-a[2] * b[4], a[1] * b[4], -a[0] * b[4],
             -a[0] * b[0] - a[1] * b[1] - a[2] * b[2]);
}

inline Sca ip(const Tnb& a, const Sta& b) { return Sca(); }

inline Sca ip(const Tnb& a, const Drv& b) { return Sca(); }
...
inline Sca ip(const Tnb& a, const Tnv& b) { return Sca(); }

inline Sca ip(const Tnb& a, const Tnb& b) { return Sca(); }

inline Sca ip(const Tnb& a, const Tnt& b) { return Sca(); }
...
inline Sca ip(const Tnb& a, const Flp& b) { return Sca(); }

inline Sca ip(const Tnb& a, const Dfp& b) {
  return Sca(a[0] * b[0] + a[1] * b[1] + a[2] * b[2]);
}

inline Sca ip(const Tnb& a, const Dll& b) { return Sca(); }

inline Sca ip(const Tnb& a, const Dlp& b) { return Sca(); }

inline Sca ip(const Tnb& a, const Vec& b) { return Sca(); }

inline Sca ip(const Tnb& a, const Biv& b) { return Sca(); }

inline Sca ip(const Tnb& a, const Tri& b) { return Sca(); }

inline Sca ip(const Tnb& a, const Aff& b) { return Sca(); }

inline Sca ip(const Tnb& a, const Afl& b) { return Sca(); }

inline Sca ip(const Tnb& a, const Afp& b) { return Sca(); }

inline Sca ip(const Tnb& a, const Dap& b) { return Sca(); }

inline Ori ip(const Tnb& a, const Daf& b) {
  return Ori(-a[0] * b[0] - a[1] * b[1] - a[2] * b[2]);
}

inline Sca ip(const Tnb& a, const Dal& b) { return Sca(); }

inline Sca ip(const Tnb& a, const Mnv& b) { return Sca(); }

template <>
inline Tnb sp(const Tnb& tnb, const Rot& rot) {
  const Rot& t1 = rot;
  const Tnb& t2 = tnb;
  Tnv_Vec t3(-t1[1] * t2[0] - t1[2] * t2[1] - t1[3] * t2[2],
             t1[0] * t2[0] - t1[2] * t2[2] + t1[3] * t2[1],
             t1[0] * t2[1] + t1[1] * t2[2] - t1[3] * t2[0],
             t1[0] * t2[2] - t1[1] * t2[1] + t1[2] * t2[0]);
  const Rot& t4 = rot;
  Rot t5 = reverse(t4);
  return Tnb(t3[0] * t5[1] + t3[1] * t5[0] - t3[2] * t5[3] + t3[3] * t5[2],
...
inline Tnb sp(const Tnb& tnb, const Mot& mot) {
  const Mot& t1 = mot;
  const Tnb& t2 = tnb;
  Par_Tnb t3(t1[5] * t2[0] + t1[6] * t2[1] + t1[7] * t2[2],
  ...
  const Mot& t4 = mot;
  Mot t5 = reverse(t4);
  return Tnb(t3[3] * t5[1] + t3[4] * t5[0] - t3[5] * t5[3] + t3[6] * t5[2],
...
inline Tnb sp(const Tnb& tnb, const Dil& dil) {
  const Dil& t1 = dil;
  const Tnb& t2 = tnb;
  Tnb t3(t1[0] * t2[0] - t1[1] * t2[0], t1[0] * t2[1] - t1[1] * t2[1],
         t1[0] * t2[2] - t1[1] * t2[2]);
  const Dil& t4 = dil;
  Dil t5 = reverse(t4);
  return Tnb(t3[0] * t5[0] + t3[0] * t5[1], t3[1] * t5[0] + t3[1] * t5[1],
             t3[2] * t5[0] + t3[2] * t5[1]);
}
...
inline Tnb sp(const Tnb& tnb, const Trv& trv) {
  const Trv& t1 = trv;
  const Tnb& t2 = tnb;
  Tnb t3(t1[0] * t2[0], t1[0] * t2[1], t1[0] * t2[2]);
  const Trv& t4 = trv;
  Trv t5 = reverse(t4);
  return Tnb(t3[0] * t5[0], t3[1] * t5[0], t3[2] * t5[0]);
}
...
inline Tnb sp(const Tnb& tnb, const Mtd& mtd) {
  const Mtd& t1 = mtd;
  const Tnb& t2 = tnb;
  Par_Tnb t3(t1[5] * t2[0] + t1[6] * t2[1] + t1[11] * t2[2],
  ...
  const Mtd& t4 = mtd;
  Mtd t5 = reverse(t4);
  return Tnb(t3[3] * t5[1] + t3[3] * t5[8] + t3[4] * t5[0] + t3[4] * t5[7] -
...
inline Tnb sp(const Tnb& tnb, const Trs& trs) {
  const Trs& t1 = trs;
  const Tnb& t2 = tnb;
  Tnv_Dfp t3(t1[2] * t2[0] + t1[3] * t2[1], -t1[1] * t2[0] + t1[3] * t2[2],
  ...
  const Trs& t4 = trs;
  Trs t5 = reverse(t4);
  return Tnb(t3[3] * t5[0], t3[4] * t5[0], t3[5] * t5[0]);
}
...
inline Tnb sp(const Tnb& tnb, const Rtc& rtc) {
  ...
  return Tnb(
      t3[0] * t5[5] + t3[1] * t5[3] - t3[2] * t5[7] + t3[3] * t5[6] -
          ...
          t3[6] * t5[14] - t3[6] * t5[15] - t3[7] * t5[5] - t3[8] * t5[3] +
          t3[9] * t5[7] - t3[10] * t5[6] - t3[11] * t5[2] - t3[11] * t5[13],
      t3[0] * t5[6] + t3[1] * t5[7] + t3[2] * t5[3] - t3[3] * t5[5] -
          t3[4] * t5[2] - t3[4] * t5[13] + t3[5] * t5[14] + t3[5] * t5[15] +
          t3[6] * t5[0] + t3[6] * t5[11] - t3[7] * t5[6] - t3[8] * t5[7] -
          t3[9] * t5[3] + t3[10] * t5[5] + t3[11] * t5[1] + t3[11] * t5[12],
      t3[0] * t5[7] - t3[1] * t5[6] + t3[2] * t5[5] + t3[3] * t5[3] -
          t3[4] * t5[14] - t3[4] * t5[15] - t3[5] * t5[2] - t3[5] * t5[13] +
          t3[6] * t5[1] + t3[6] * t5[12] - t3[7] * t5[7] + t3[8] * t5[6] -
          t3[9] * t5[5] - t3[10] * t5[3] - t3[11] * t5[0] - t3[11] * t5[11]);
}
...
inline Tnb sp(const Tnb& tnb, const Mtt& mtt) {
  const Mtt& t1 = mtt;
  const Tnb& t2 = tnb;
  Par_Tnb t3(t1[8] * t2[0] + t1[9] * t2[1] + t1[15] * t2[2],
  ...
  return Tnb(
      t3[0] * t5[5] - t3[1] * t5[4] + t3[2] * t5[14] + t3[3] * t5[1] +
          ...
          t3[5] * t5[13] + t3[6] * t5[2] + t3[6] * t5[12] - t3[7] * t5[5] +
          t3[8] * t5[4] - t3[9] * t5[14] + t3[10] * t5[6] - t3[11] * t5[6],
      t3[0] * t5[6] - t3[1] * t5[14] - t3[2] * t5[4] + t3[3] * t5[2] +
          t3[3] * t5[12] + t3[4] * t5[3] + t3[4] * t5[13] + t3[5] * t5[0] +
          t3[5] * t5[10] - t3[6] * t5[1] - t3[6] * t5[11] - t3[7] * t5[6] +
          t3[8] * t5[14] + t3[9] * t5[4] - t3[10] * t5[5] + t3[11] * t5[5],
...
inline Tnb sp(const Tnb& tnb, const Rtd& rtd) {
  const Rtd& t1 = rtd;
  const Tnb& t2 = tnb;
  Tnv_Vec t3(-t1[1] * t2[0] - t1[2] * t2[1] - t1[3] * t2[2] + t1[5] * t2[0] +
                 t1[6] * t2[1] + t1[7] * t2[2],
  ...
  const Rtd& t4 = rtd;
  Rtd t5 = reverse(t4);
  return Tnb(t3[0] * t5[1] + t3[0] * t5[5] + t3[1] * t5[0] + t3[1] * t5[4] -
...
inline Tnb sp(const Tnb& tnb, const Tvd& tvd) {
  const Tvd& t1 = tvd;
  const Tnb& t2 = tnb;
  Tnb t3(t1[0] * t2[0] - t1[4] * t2[0], t1[0] * t2[1] - t1[4] * t2[1],
         t1[0] * t2[2] - t1[4] * t2[2]);
  const Tvd& t4 = tvd;
  Tvd t5 = reverse(t4);
  return Tnb(t3[0] * t5[0] + t3[0] * t5[4], t3[1] * t5[0] + t3[1] * t5[4],
             t3[2] * t5[0] + t3[2] * t5[4]);
}
...
inline Tnb sp(const Tnb& tnb, const Tsd& tsd) {
  ...
  const Tsd& t4 = tsd;
  Tsd t5 = reverse(t4);
  return Tnb(t3[3] * t5[0] + t3[3] * t5[4], t3[4] * t5[0] + t3[4] * t5[4],
             t3[5] * t5[0] + t3[5] * t5[4]);
}
...
inline Tnb sp(const Tnb& tnb, const Trt& trt) {
  const Trt& t1 = trt;
  const Tnb& t2 = tnb;
  Par_Tnb t3(t1[8] * t2[0] + t1[9] * t2[1], -t1[7] * t2[0] + t1[9] * t2[2],
             -t1[7] * t2[1] - t1[8] * t2[2],
             -t1[1] * t2[0] - t1[2] * t2[1] - t1[3] * t2[2] + t1[11] * t2[0] +
                 t1[12] * t2[1] + t1[13] * t2[2],
             t1[0] * t2[0] - t1[2] * t2[2] + t1[3] * t2[1] - t1[10] * t2[0] +
  ...
  return Tnb(
      t3[0] * t5[5] - t3[1] * t5[4] + t3[3] * t5[1] + t3[3] * t5[11] +
          ...
          t3[6] * t5[2] + t3[6] * t5[12] - t3[7] * t5[5] + t3[8] * t5[4] +
          t3[10] * t5[6] - t3[11] * t5[6],
      t3[0] * t5[6] - t3[2] * t5[4] + t3[3] * t5[2] + t3[3] * t5[12] +
          t3[4] * t5[3] + t3[4] * t5[13] + t3[5] * t5[0] + t3[5] * t5[10] -
          t3[6] * t5[1] - t3[6] * t5[11] - t3[7] * t5[6] + t3[9] * t5[4] -
          t3[10] * t5[5] + t3[11] * t5[5],
      t3[1] * t5[6] - t3[2] * t5[5] + t3[3] * t5[3] + t3[3] * t5[13] -
          t3[4] * t5[2] - t3[4] * t5[12] + t3[5] * t5[1] + t3[5] * t5[11] +
          t3[6] * t5[0] + t3[6] * t5[10] - t3[8] * t5[6] + t3[9] * t5[5] +
          t3[10] * t5[4] - t3[11] * t5[4]);
}
...
inline Tnb sp(const Tnb& tnb, const Rvd& rvd) {
  const Rvd& t1 = rvd;
  const Tnb& t2 = tnb;
  Tnv_Vec t3(-t1[1] * t2[0] - t1[2] * t2[1] - t1[3] * t2[2] + t1[8] * t2[0] +
                 t1[9] * t2[1] + t1[10] * t2[2],
             t1[0] * t2[0] - t1[2] * t2[2] + t1[3] * t2[1] - t1[7] * t2[0] +
                 t1[9] * t2[2] - t1[10] * t2[1],
             t1[0] * t2[1] + t1[1] * t2[2] - t1[3] * t2[0] - t1[7] * t2[1] -
  ...
  const Rvd& t4 = rvd;
  Rvd t5 = reverse(t4);
  return Tnb(t3[0] * t5[1] + t3[0] * t5[8] + t3[1] * t5[0] + t3[1] * t5[7] -
...
inline Tnb sp(const Tnb& tnb, const Tvt& tvt) {
  const Tvt& t1 = tvt;
  const Tnb& t2 = tnb;
  Par_Tnb t3(t1[8] * t2[0] + t1[9] * t2[1] + t1[14] * t2[2],
...
inline Tnb sp(const Tnb& tnb, const Bst& bst) {
  ...
  return Tnb(
      t3[0] * t5[5] - t3[1] * t5[4] + t3[3] * t5[1] + t3[4] * t5[0] +
          t3[4] * t5[10] - t3[5] * t5[3] + t3[6] * t5[2] - t3[7] * t5[5] +
          t3[8] * t5[4] + t3[10] * t5[6] - t3[11] * t5[6],
      t3[0] * t5[6] - t3[2] * t5[4] + t3[3] * t5[2] + t3[4] * t5[3] +
          t3[5] * t5[0] + t3[5] * t5[10] - t3[6] * t5[1] - t3[7] * t5[6] +
          t3[9] * t5[4] - t3[10] * t5[5] + t3[11] * t5[5],
      t3[1] * t5[6] - t3[2] * t5[5] + t3[3] * t5[3] - t3[4] * t5[2] +
          t3[5] * t5[1] + t3[6] * t5[0] + t3[6] * t5[10] - t3[8] * t5[6] +
          t3[9] * t5[5] + t3[10] * t5[4] - t3[11] * t5[4]);
}
...
inline Tnb re(const Tnb& tnb, const Vec& vec) {
  ...
  const Tnb& t2 = tnb;
  Tnb t3 = involute(t2);
  Tnv_Biv t4(-t1[1] * t3[0] - t1[2] * t3[1], t1[0] * t3[0] - t1[2] * t3[2],
             t1[0] * t3[1] + t1[1] * t3[2],
             t1[0] * t3[2] - t1[1] * t3[1] + t1[2] * t3[0]);
  const Vec& t5 = vec;
  Vec t6 = reverse(t5);
  return Tnb(-t4[0] * t6[1] + t4[1] * t6[0] - t4[3] * t6[2],
...
inline Tnb re(const Tnb& tnb, const Pln& pln) {
  ...
  const Tnb& t2 = tnb;
  Tnb t3 = involute(t2);
  Ori_Par t4(t1[3] * t3[2], -t1[3] * t3[1], t1[3] * t3[0],
  ...
  const Pln& t5 = pln;
  Pln t6 = reverse(t5);
  return Tnb(t4[3] * t6[0] - t4[5] * t6[2] + t4[6] * t6[1],
...
inline Tnb re(const Tnb& tnb, const Dll& dll) {
  ...
  const Tnb& t2 = tnb;
  Tnb t3 = involute(t2);
  Par_Tnb t4(t1[4] * t3[0] + t1[5] * t3[1], -t1[3] * t3[0] + t1[5] * t3[2],
             -t1[3] * t3[1] - t1[4] * t3[2],
             -t1[0] * t3[0] - t1[1] * t3[1] - t1[2] * t3[2],
             -t1[1] * t3[2] + t1[2] * t3[1], t1[0] * t3[2] - t1[2] * t3[0],
...
inline Tnb re(const Tnb& tnb, const Lin& lin) {
  ...
  const Tnb& t2 = tnb;
  Tnb t3 = involute(t2);
  Rvd t4(t1[0] * t3[0] + t1[1] * t3[1] + t1[2] * t3[2],
         t1[1] * t3[2] - t1[2] * t3[1], -t1[0] * t3[2] + t1[2] * t3[0],
         t1[0] * t3[1] - t1[1] * t3[0], t1[4] * t3[0] + t1[5] * t3[1],
         -t1[3] * t3[0] + t1[5] * t3[2], -t1[3] * t3[1] - t1[4] * t3[2],
         t1[0] * t3[0] + t1[1] * t3[1] + t1[2] * t3[2],
  ...
  const Lin& t5 = lin;
  Lin t6 = reverse(t5);
  return Tnb(-t4[4] * t6[4] + t4[5] * t6[3] - t4[11] * t6[5],
...
inline Tnb re(const Tnb& tnb, const Pnt& pnt) {
  ...
  const Tnb& t2 = tnb;
  Tnb t3 = involute(t2);
  Ori_Cir t4(-t1[4] * t3[0], -t1[4] * t3[1], -t1[4] * t3[2],
             -t1[1] * t3[0] - t1[2] * t3[1], t1[0] * t3[0] - t1[2] * t3[2],
             t1[0] * t3[1] + t1[1] * t3[2], -t1[4] * t3[0], -t1[4] * t3[1],
             -t1[4] * t3[2], t1[0] * t3[2] - t1[1] * t3[1] + t1[2] * t3[0]);
  const Pnt& t5 = pnt;
  Pnt t6 = reverse(t5);
  return Tnb(t4[0] * t6[3] - t4[3] * t6[1] + t4[4] * t6[0] - t4[6] * t6[3] -
                 t4[9] * t6[2],
...
inline Tnb re(const Tnb& tnb, const Par& par) {
  ...
  const Tnb& t2 = tnb;
  Tnb t3 = involute(t2);
  Par_Tnb t4(t1[7] * t3[0] + t1[8] * t3[1], -t1[6] * t3[0] + t1[8] * t3[2],
             -t1[6] * t3[1] - t1[7] * t3[2],
             -t1[0] * t3[0] - t1[1] * t3[1] - t1[2] * t3[2],
  ...
  const Par& t5 = par;
  Par t6 = reverse(t5);
  return Tnb(t4[0] * t6[4] - t4[1] * t6[3] + t4[3] * t6[0] + t4[4] * t6[9] -
                 t4[5] * t6[2] + t4[6] * t6[1] - t4[7] * t6[4] + t4[8] * t6[3] +
                 t4[10] * t6[5] - t4[11] * t6[5],
             t4[0] * t6[5] - t4[2] * t6[3] + t4[3] * t6[1] + t4[4] * t6[2] +
                 t4[5] * t6[9] - t4[6] * t6[0] - t4[7] * t6[5] + t4[9] * t6[3] -
                 t4[10] * t6[4] + t4[11] * t6[4],
             t4[1] * t6[5] - t4[2] * t6[4] + t4[3] * t6[2] - t4[4] * t6[1] +
                 t4[5] * t6[0] + t4[6] * t6[9] - t4[8] * t6[5] + t4[9] * t6[4] +
...
inline Tnb re(const Tnb& tnb, const Cir& cir) {
  ...
  const Cir& t5 = cir;
  Cir t6 = reverse(t5);
  return Tnb(t4[0] * t6[0] - t4[2] * t6[2] + t4[3] * t6[1] - t4[4] * t6[7] +
                 t4[5] * t6[6] - t4[6] * t6[9] - t4[7] * t6[0] + t4[9] * t6[2] -
                 t4[10] * t6[1] - t4[11] * t6[8],
             t4[0] * t6[1] + t4[1] * t6[2] - t4[3] * t6[0] - t4[4] * t6[8] +
                 t4[5] * t6[9] + t4[6] * t6[6] - t4[7] * t6[1] - t4[8] * t6[2] +
                 t4[10] * t6[0] + t4[11] * t6[7],
             t4[0] * t6[2] - t4[1] * t6[1] + t4[2] * t6[0] - t4[4] * t6[9] -
                 t4[5] * t6[8] + t4[6] * t6[7] - t4[7] * t6[2] + t4[8] * t6[1] -
                 t4[9] * t6[0] - t4[11] * t6[6]);
}
...
inline Tnb re(const Tnb& tnb, const Sph& sph) {
  ...
  const Tnb& t2 = tnb;
  Tnb t3 = involute(t2);
  Ori_Par t4(t1[4] * t3[2], -t1[4] * t3[1], t1[4] * t3[0],
             t1[0] * t3[0] + t1[1] * t3[1] + t1[2] * t3[2],
             t1[1] * t3[2] - t1[2] * t3[1], -t1[0] * t3[2] + t1[2] * t3[0],
             t1[0] * t3[1] - t1[1] * t3[0], t1[4] * t3[2], -t1[4] * t3[1],
             t1[4] * t3[0]);
  const Sph& t5 = sph;
  Sph t6 = reverse(t5);
  return Tnb(t4[2] * t6[3] + t4[3] * t6[0] - t4[5] * t6[2] + t4[6] * t6[1] -
                 t4[9] * t6[3],
