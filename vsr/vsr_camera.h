...
struct Frustrum {
  ...
  Pln left() {
    return Ro::null(box.tl) ^ Ro::null(box.bl) ^ Ro::null(box.blb) ^ Inf(1);
  }
  Pln right() {
    return Ro::null(box.tr) ^ Ro::null(box.br) ^ Ro::null(box.brb) ^ Inf(1);
  }
  Pln top() {
    return Ro::null(box.tl) ^ Ro::null(box.tr) ^ Ro::null(box.trb) ^ Inf(1);
  }
  Pln bottom() {
    return Ro::null(box.bl) ^ Ro::null(box.br) ^ Ro::null(box.brb) ^ Inf(1);
  }
};
...
class Camera : public Frame {
 ...
 public:
  ...
  //		void mvm( const Rot& r ) { mModelView.rot(r); }				///<
  //Set Model View Matrix Rotor 		Rot& mvm() { return mModelView.rot(); }
  /////< Model View Matrix Rotor 		Rot  mvm()  const { return mModelView.rot(); }
  /////< Get Model View Matrix Rotor 		Rot  cat()  const { return mRot *
  //mModelView.rot(); }		///< Concatenate MVM and Camera Rotor 		Rot rcat()
  //const { return mModelView.rot() * mRot; }		///< Reverse Concatenate
  //MVM and Camera Rotor (not used) 		Frame& modelView() { return mModelView; }
  /////< Get Model View Matrix Frame 		Frame  modelView() const { return
  //mModelView; }				///< Set Model View Matrix Frame

  //		Rot rat() const { return !mRot * mModelView.rot(); }		///<
  //Ratio Rotor taking camera to mvm 		Rot rrat() const { return !mModelView.rot()
  //* mRot; }		///< Reverse Ratio taking mvm to camera

  //		Rot ryz() { return Gen::ratio( modelView().yz(), yz() ); }	///<
  //Get Ratio of YZ plane between Camera and MVM 		Rot rxz() { return Gen::ratio(
  //modelView().xz(), xz() ); }	///< Get Ratio of XZ plane between Camera and
  //MVM 		Rot rxy() { return Gen::ratio( modelView().xy(), xy() ); }	///< Get
  //Ratio of XY plane between Camera and MVM
  ...
  //} 		Biv xzcat(){ return xz() + mModelView.xz(); }//Op::sp0(xz(),-mvm()); } 		Biv
  //yzcat(){ return yz() + mModelView.yz(); }//Op::sp0(yz(),-mvm()); }
  ...
  Frustrum frustrum() {
    ...
    //           mFrustrum.box.tr = GL::unproject( width(), height(), 0.0,
    //           *this); mFrustrum.box.trb = GL::unproject( width(),
    //           height(), 1.0, *this ); mFrustrum.box.br =
    //           GL::unproject(width(),0,0.0, *this); mFrustrum.box.brb =
    //           GL::unproject(width(),0,1.0, *this); mFrustrum.box.tl =
    //           GL::unproject( 0, height(), 0.0, *this ); mFrustrum.box.tlb =
    //           GL::unproject( 0,height(), 1.0, *this );
...
//
//            Rot cat() { return camera.rot() * model.rot(); }//camera.rot() *
//            model.rot(); }
//
//            Mat4f mod() { return model.image(); }
//
//            Mat4f mvm() { return XMat::lookAt( camera.x(), camera.y(),
//            camera.z() * -1, camera.pos()) * XMat::rot( model.rot() ) ; }
//
//            Mat4f proj() {
//                Lens& tl = camera.lens();
//                return XMat::fovy( tl.mFocal * PI/180.0, tl.mWidth/tl.mHeight,
//                tl.mNear, tl.mFar );
//            }
//            Mat4f norm(){
...
//
//                Mat4f tmod = mod();
//                Mat4f tview = XMat::lookAt( camera.x(), camera.y(), camera.z()
//                * -1, camera.pos()); Mat4f tmvm = mvm(); Mat4f tproj = proj();
//                Mat4f tnorm = norm();
...
class Scene {
 public:
  ...
  Rot cat() {
    return camera.rot() * model.rot();
  }  // camera.rot() * model.rot(); }

  Mat4f mod() { return model.image(); }

  Mat4f mvm() {
    return XMat::lookAt(camera.x(), camera.y(), camera.z() * -1, camera.pos()) *
           XMat::rot(model.quat());
  }

  Mat4f proj() {
    Lens& tl = camera.lens();
    return XMat::fovy(tl.mFocal * PI / 180.0, tl.mWidth / tl.mHeight, tl.mNear,
                      tl.mFar);
  }
  ...
  void updateMatrices() {
    Mat4f tmod = mod();
    Mat4f tview =
        XMat::lookAt(camera.x(), camera.y(), camera.z() * -1, camera.pos());
    Mat4f tmvm = mvm();
    Mat4f tproj = proj();
    Mat4f tnorm = norm();
    Mat4f tmvp = tproj * tmvm;
