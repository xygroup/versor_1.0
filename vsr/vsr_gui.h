...
enum { SLIDER, SLIDERS, SLIDER2D, BUTTON, BUTTONS, DIALER, COLORSLIDER };
...
class Gui : public Table {
 ...
 public:
  ...
  void add(Widget* v) {
    ...
    // cout << "adding and placing pointer " << (v->name()) << "to gui." <<
    // endl;
  ...
  //				template <class V>
  //				void add (int, string, callback, V& ob, float min = 0.,
  //float max = 1.,int numx = 0, int numy = 0 );
...
void Gui ::add(int _type, string _name, V& val, float min, float max) {
  ...
  switch (_type) {
    ...
    case COLORSLIDER: {
      //				ColorSliders * ns =	new
      //ColorSliders( Rect(300,50) ); 				s = (Widget*)ns;
      break;
    }
  }
...
//	template <class V>
//	void Gui :: add (int _type, string _name, callback cb, V& ob, float min
//, float max, int numx , int numy ){
//
//		Widget * s;
//		switch (_type){
//			case SLIDER:
...
//			{
//				NumberDialer * ns = new
//NumberDialer(4,3,max,min); 				ns -> attach(cb, Update::Value, &ob);
//
//				s = (Widget*)ns;
//				break;
//			}
//			case SLIDERS:
//			{
//				Sliders * ns =	new Sliders( Rect(100,20), numx,
//numy, 0 ); 				ns -> attach(cb, Update::Value, &ob); 				s = (Widget*)ns; 				break;
//			}
//		}
//
