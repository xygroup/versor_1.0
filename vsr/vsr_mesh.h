...
struct Mesh {  //: public Frame {
 ...
 public:
  ...
  Mesh& useElements(bool b) {
    bUseElements = b;
    return *this;
  }
  Mesh& useColor(bool b) {
    bUseColor = b;
    return *this;
  }
  Mesh& useNormals(bool b) {
    bUseNormals = b;
    return *this;
  }

  /// Default Line Loop Mode
  Mesh(GL::MODE m = GL::LL, bool bE = false, bool bC = false)
      : mMode(m), bUseElements(bE), bUseColor(bC), bUseNormals(false) {}
  ...
  Mesh& add(const Vec& p, const Vec& n) {
    mVertex.push_back(Vertex(Vec3f(p), Vec3f(n)));
    return *this;
  }
  Mesh& add(const Vec& p, float r, float g, float b, float a = 1.0) {
    mVertex.push_back(Vertex(Vec3f(p), Vec3f(0, 0, 1), Vec4f(r, g, b, a)));
    return *this;
  }

  Mesh& add(float x, float y, float z) {
    mVertex.push_back(Vertex(Vec3f(x, y, z)));
    return *this;
  }
  // Mesh& add(const Vec& v) { mVertex.push_back( Vertex( Vec3f(v[0], v[1],
  // v[2]) ) ); return *this; }
  ...
  Vec3f at(double t) {
    ...
    return Interp::linear<Vec3f>(mVertex[idx].Pos, mVertex[idx + 1].Pos,
                                 remainder);
  }
  ...
  void draw(float r = 1.0, float g = 1.0, float b = 1.0, float a = 1.0) {
    ...
    if (!bUseColor) {
      if (bUseElements) {
        if (bUseNormals)
          drawElementsNormal(r, g, b, a);
        else
          drawElements(r, g, b, a);
      } else {
        if (bUseNormals)
          drawVerticesNormal(r, g, b, a);
        else
          drawVertices(r, g, b, a);
      }
    } else {
      if (bUseElements)
        drawElementsColor();
  ...
  void drawVertices(float r = 1.0, float g = 1.0, float b = 1.0,
                    float a = 1.0) {
    GL::Begin(mMode);
  ...
  void drawVerticesNormal(float r = 1.0, float g = 1.0, float b = 1.0,
                          float a = 1.0) {
    GL::Begin(mMode);
    for (int i = 0; i < mVertex.size(); ++i) {
  ...
  void drawElements(float r = 1.0, float g = 1.0, float b = 1.0,
                    float a = 1.0) {
    GL::Begin(mMode);
  ...
  void drawElementsNormal(float r = 1.0, float g = 1.0, float b = 1.0,
                          float a = 1.0) {
    GL::Begin(mMode);
    for (int i = 0; i < mIndex.size(); ++i) {
...
inline Mesh Mesh::Sphere(double rad, int slices, int stacks) {
  ...
  for (int i = 0; i < stacks - 1; ++i) {
    ...
    for (int j = 0; j < slices; ++j) {
      ...
      int b = (i < stacks - 2) ? a + slices
                               : m.num() - 1;  // Higher Latitue or North Pole
...
inline Mesh Mesh::Skin(T cir, int num, int res) {
  ...
  for (int i = 0; i < res; ++i) {
    for (int j = 0; j < num - 1; ++j) {
      ...
      m[a].Norm = tc.unit();
      // cout << m[a].Norm[0] << " " << m[a].Norm[1] << " " <<  m[a].Norm[2] <<
      // endl;
    }
  }
...
struct UVMesh {
  ...
  struct Quad {
    int a, b, c, d;
    Vec normal;
    Quad(const Vec& v, int _a, int _b, int _c, int _d)
        : normal(v), a(_a), b(_b), c(_c), d(_d) {}
  };
  ...
  Vec normal(int a, int b, int c) {
    Vec dir = bFlipNormals ? Ro::dir(vp[a] ^ vp[c] ^ vp[b]).dual()
                           : Ro::dir(vp[a] ^ vp[b] ^ vp[c]).dual();
    return dir.unit();
  }
