...
struct OpticalSurface {
  OpticalSurface(double r = 1.5, Cir c = CXY(1))
      : ridx(r), cir(c), convex(true) {}

  void set(double r, Cir c, bool con) {
    ridx = r;
    cir = c;
    convex = con;
  }
  ...
  Pnt meet(const Cir& c) {
    ...
    Cir tc = Ro::sur(c) ^ cir.dual();  //( Ro::car ( ( Ro::sur(c) ^ Ro::sur(cir)
                                       //).dual() ) ).dual() ^ cir.dual();

    return ((convex && sign) || (!convex && !sign)) ? Ro::split2(tc.dual())
                                                    : Ro::split1(tc.dual());

    //            return (convex || !sign ) ? Ro::split2( tc.dual() ) :
    //            Ro::split1( tc.dual() ) ;
  }
  ...
  Par tangent(const Dll& ray) { return Ta::at(ray.re(cir), meet(ray), true); }

  /// Tangent of incoming curved light ray
  Par tangent(const Cir& c) { return Ta::at(c.re(cir), meet(c), false); }

  /// Reflection of a Straight Ray
  Dll reflect(const Dll& ray) { return (tangent(ray) ^ Inf(1)).dual(); }

  /// Reflection of a Circle (straight ray output)
  Dll reflect(const Cir& c) { return (tangent(c) ^ Inf(1)).dual(); }
  ...
  Dll refract(const Dll& ray, bool exit = false) {
    ...
    return (exit) ? nray.mot(rat * (1 - ridx) * (PI / 4.0 - norm))
                  : nray.mot(rat * (ridx - 1) * (PI / 4.0 - norm));
  }

  /// Refract a circle back to a straight line by refracting the tangent line at
  /// the meet
  ...
  /// Bend incoming straight line ray by amt (refracts the ray first by ridx,
  /// then bends it by amt)
  ...
  /// Bend incoming curved line ray by amt (refracts the ray first by ridx, then
  /// bends it by amt)
...
// 3d extrapolation
struct OptSurf {
  OptSurf(double r = 1.5, Dls s = Ro::dls3(0, 0, 0))
      : ridx(r), dls(s), rad(1), convex(true) {}

  void set(double r, Dls s, bool con) {
    ridx = r;
    dls = s;
    convex = con;
  }
  ...
  /// Meet of Curved Ray (split and decide which point to use later, or use for
  /// tangent)
  ...
  /// Tangent of incoming curved light ray
  Par tangent(const Cir& c) const { return Ta::at(c.re(dls), meet(c), false); }

  /// Reflection of a Straight Ray
  Dll reflect(const Dll& ray) const { return (tangent(ray) ^ Inf(1)).dual(); }

  /// Reflection of a Circle (straight ray output)
  Dll reflect(const Cir& c) const { return (tangent(c) ^ Inf(1)).dual(); }
...
struct Lenz : public Frame {
  ...
  Lenz(Type s = Biconvex, bool g = false)
      : type(s), gradient(g), mDist(1), mGrad(0) {
    position();
  }
  ...
  Lenz(const Frame& f, float r, float g, float dist, float rat)
      : type(Biconvex), gradient(0), mGrad(0), mDist(1) {
    set(f, r, g, dist, rat);
  }
  ...
  void position() {
    switch (type) {
      ...
      case Biconcave:
        ...
        //                    enter().cir = enter().cir.trs( -r1 - (dist/2.0),
        //                    0, 0 ) ; exit().cir = exit().cir.trs( r2 +
        //                    dist/2.0, 0, 0 ) ;
        break;
      default:
        break;
  ...
  void drawMeet() { meet.draw(0, 0, 0); }

  void drawLeft() {
    //            Draw::SegPnts(
  }

  void drawPath() { Glyph::Line(Vec(pa), Vec(pb)); }

  void drawBend() { Draw::SegPnts(bend, pa, pb); }
...
struct Trajectory {
  ...
  struct CirSeg {
    CirSeg(const Cir& c, const Pnt& _pa, const Pnt& _pb)
        : cir(c), pa(_pa), pb(_pb) {}
    Cir cir;
    Pnt pa, pb;
  };
  ...
  //                switch( seg[i] -> id ){
  //                    case _CIR:
  //                    Draw::SegPnts(seg[i] -> states[0], seg[i] -> states[1],
  //                    seg[i] -> states[2] );
  //                     break;
  //                    case _LIN:
  //                    Glyph::Line( Vec( seg[i] -> states[0] ), Vec ( seg[i] ->
  //                    states[1] ) ); break;
  //                }
  //            END
  //        }
  void draw() {
    ...
    it++;
    END if (bMet) Glyph::Line(Vec(linseg[it].pa), Vec(linseg[it].pb));
  ...
  void print(const Camera& cam) {
    ...
    //  s += Print::PlotEnd();
    it++;
    END s += Print::Line(linseg[it].pa, linseg[it].pb, cam);
...
struct Lens {
  ...
  Lens(Type s = Biconvex, bool g = false)
      : type(s), gradient(g), dist(.5), grad(0) {
    position();
  }
  ...
  Lens& trs(const State& s) {
    position();
    ITJ(i, 2) os[i].cir = os[i].cir.trs(s);
    END structure();
  }
  ...
  Dll pathIn(const Dll& ray) { return enter().refract(ray); }

  Dll pathOut(const Dll& ray) { return exit().refract(ray, true); }
...
//        Pnt pa = Ro::null ( ( ray.conjugate() ^ Dlp(1,0,0).trs(-2,0,0)
//        ).dual().runit() );
//
...
//        Pnt pout = Ro::null ( ( nd2.conjugate() ^ Dlp(1,0,0).trs(2,0,0)
//        ).dual().runit() ) ; Glyph::Line(Vec( lens2.meet(nd) ) , Vec( pout )
//        );
...
//        Lin nor = Ta::at(c, tp,0) ^ Inf(1);  //Drv d = Fl::dir(nor,0);
//        d.draw();
//        //Ratio between line tangent and incoming ray
...
//        //REFLECTION OF LIGHT RAY IS TANGENT TO ITS INVERSION IN CIRCLE (Seems
//        to me anyway) Par tan = Ta::at(nl, Ro::null(tp),0); Lin reflection =
//        tan ^ Inf(1);
