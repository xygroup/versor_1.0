...
/*
                ...
                GLint compileStatus;
                glGetObjectParameterivARB( shader, GL_COMPILE_STATUS,
   &compileStatus ); glGetObjectParameterivARB( program, GL_INFO_LOG_LENGTH,
   &infoLogLen ); infoLog = new GLchar[infoLogLen+1];
...
class ShaderProgram {
 ...
 public:
  ...
  void load(
      const char* shaderName);  ///< Load Shader From File(.frag and .vert)
  ...
  //            if (loc == -1) printf("No such uniform named \"%s\" in
  //            shader\n", name);
  //
  ...
  //////////////////
  ////THE FOLLOWING STATIC FUNCTIONS OPERATE ON WHATEVER SHADER IS CURRENTLY
  ///BOUND
  //////////////////
...
Shader ::Shader(string shaderName, Shader::Type t)
...
inline void ShaderProgram::get() {
  ...
  for (int j = 0; j < numActiveUniforms; j++) {
    ...
    //            if(u.name[ strlen(u.name)-3 ] == '[' && u.name[
    //            strlen(u.name)-1 ] == ']') {
