...
class Lattice {
 public:
  Lattice(int n) : mNum(n) { mPoint = new Pnt[mNum]; }

  int num() const { return mNum; }
  int num(int n) { resize(n); }
...
class CubicLattice {
 public:
  ...
  void init() {
    ITER mPoint[tidx] = Ro::null(px(i), py(j), pz(k));
    ...
    Vec v(px(i) + mSpacing / 2.0, py(j) + mSpacing / 2.0,
          pz(k) - mSpacing / 2.0);
  ...
  Vec bound(const Vec& p) const {
    ...
    if (v[0] < px(0))
      v[0] = px(0);  // + f.spacing()/2.0;
    else if (v[0] > px(w() - 1))
      v[0] = px(w() - 1);  // - f.spacing()/2.0;

    if (v[1] < py(0))
      v[1] = py(0);  // + f.spacing()/2.0;
    else if (v[1] > py(h() - 1))
      v[1] = py(h() - 1);  // - f.spacing()/2.0;

    if (v[2] > pz(0))
      v[2] = pz(0);  // - f.spacing()/2.0;
    else if (v[2] < pz(d() - 1))
      v[2] = pz(d() - 1);  // + f.spacing()/2.0;
  ...
  Pnt gridAt(int w = 0, int h = 0, int d = 0) const {
