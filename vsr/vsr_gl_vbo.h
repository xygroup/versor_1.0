...
/// Vertex Buffer Object. Vertex data can be interleaved with color info, etc,
/// watch for packing bit alignments!
class VBO {
 private:
  ...
  GL::BUFFER mTarget;  ///< Specifies the target buffer object being mapped
                       ///< (vertex or index? or pixel (un)pack)
 ...
 public:
  ...
  //            VBO(GLvoid * udata, int num,  GLsizeiptr s, GL::BUFFER =
  //            GL::VERTEXBUFFER, GL::USAGE use = GL::STATIC);
  ...
  void drawElements(GLenum mode = GL_TRIANGLES, int num = -1, int off = 0) {
    glDrawElements(mode, (num == -1) ? mNum - off : num, GL::UINT,
                   (GLvoid*)(sizeof(GLuint) * off));
