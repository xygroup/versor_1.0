...
//  Versor
/*
    KNOTS -- building up from Dorst and Valkenburg's paper on Square Roots of
   Rotors and Logarithms through Polar Decomposition


    IN PROGRESS!!!! Still owkring out the best way to do this (and what "this"
   is)
...
struct TorusKnot {
  ...
  int iter() {
    return (P == 0 || Q == 0) ? 1.0 / amt
                              : P * Q / (amt * Ro::size(HF.cir(), false));
  }
  ...
  Bst bst() { return Gen::bst(par() * amt / 2.0); }
    ...
    for (int i = 0; i < num; ++i) {
      ...
      for (int j = 0; j < num; ++j) {
        if (i != j) {
          ...
          double diffB = fabs((totalA - globalA[j]) +
                              globalA[i]);  // fabs( globalB[j] - globalA[i] );

          double diff = diffA < diffB ? diffA : diffB;

          double tener = ((1.0 / chord) - (1.0 / (diff * diff))) *
                         local[j];  // weighted by distance
          // push back for i = 0
...
class Orbit : public Frame {
 ...
 public:
  Orbit()
      : Frame(), sF(PT(0, 0, 0)), mM(1), mAmt(.01), bReal(false), bNull(false) {
    calc();
  }
...
class Knot : public Frame {
 ...
 public:
  ...
  void calc() {
    ...
    mFa.mot(ma);
    mFb.mot(mb);
    mPar = pa(bRealA) * (mM == 0 ? 0 : PI / mM) +
           pb(bRealB) * (mN == 0 ? 0 : PI / mN);
  }
