...
/*
    ...
    FOR BUILDING GLSL SHADER PROGRAMS UP FROM COMMONLY USED SMALLER PIECES OF
   CODE
...
string AVertex =
    STRINGIFY(attribute vec3 position; attribute vec3 normal;
              attribute vec4 sourceColor; attribute vec2 texCoord;);

string UMatrix = STRINGIFY(
    //           uniform mat4 modelView;         // Model * View
    //           uniform mat4 submodel;          // Glyph-specific Transform
    //            uniform mat4 projection;        // Projection Matrix (ortho or
    //            frustrum)
...
string ULight = STRINGIFY(uniform vec3 lightPosition;);

string ULightProperty =
    STRINGIFY(uniform vec3 ambientColor; uniform vec3 diffuseMaterial;
              uniform vec3 specularColor;);

string USampler = STRINGIFY(uniform sampler2D sampleTexture;);

string USamplerCube = STRINGIFY(uniform samplerCube sampleTexture;);

// Pass from Vertex to Fragment Shader
string Varying = STRINGIFY(varying vec4 colorDst; varying vec2 texco;);

string VColor = STRINGIFY(varying vec4 colorDst;);

string VTex = STRINGIFY(varying vec2 texco;);
//
...
string NTransform = STRINGIFY(
    vec3 doNormal(vec4 n) { return normalize((normalMatrix * n).xyz); });

string VPass = STRINGIFY(
    vec4 doVertex(vec4 v) { return gl_ModelViewProjectionMatrix * v; });

string VCalc = STRINGIFY(vec4 doVertex(vec4 v) {
  mat4 m =
      modelViewProjection;  // * submodel;//projection * modelView * submodel;
  return m * v;
});
...
string VMakeCubemap = STRINGIFY(
    ...
    uniform cmFace; varying vec3 vTexDir;
    ...
    vec4 cubemap(vec4 vertex) {
      ...
      vertex.zw =
          vec2((vertex.z * (uFar + uNear) + vertex.w * uFar * uNear * 2.) /
                   (uNear - uFar),
               -vertex.z);
      return vertex;
    });
...
string FUseCubemap =
    STRINGIFY(uniform samplerCube cubeMap; varying vec3 vTexDir;

              void main(void){textureCube(cubeMap, vTexDir).rgb});
