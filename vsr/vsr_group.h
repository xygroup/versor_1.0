 ...
 *
    ...
    than attaching the point group to the lattice:
        ...
        [see this paper: "The Crystallographic Space Groups in Geometric
 Algebra" by David Hestenes and Jeremy Holt (journal of Mathematical Physics,
 January 2007]

        3) Thus Point Groups and Space Groups are uniquely defined by their
 generating versors 2D groups require a,b 3D groups require a,b,c

    4) The BAR over the p or q determines whether the transformation is made up
 of even or odd number of generatrors (bar = even)

        5) Numbers p and q in Crystallographic tables are exponentials of
 rotations and reflections and specify how many times to apply the rotation or
 reflection (i.e. 'p' times or 'q' times) Used in particular to specify the
 THETA of the ROTOR since THETA = 180/p  (or THETA = 180/q)
        ...
        6) Each space group symbol designates
                ...
                joining constraints (glide reflections or glide rotations
 (screws) )
        ...
        8) Symmorphic space groups don't have joining constraints, so reflection
 and rotation is independent of translation otherwise, in non-symmorphic space
 groups, you have "glide-reflections" and "glide-rotations" (aka screws)
    ...
    Glides ("g") are translators * reflections etc.
        ...
        when bars are present, and rotors are even, the angle between a,b, is
 180/p and btwn b, c is 180/q.  the angle between c and a is always 90 (Pi/2) so
 we don't worry about it...

        In the classical literature "m" signifies use of the pin group (aka
 reflections) in the generating algebra of the Space Group (and the Pin Group
        are normals to planes of reflection)  In the versor model of GA, it
 means you are using a collection of odd versors p or q times.

        Basically:
        1 - need something to represent the versors themselves ( a bunch of dual
 planes ) (vers2, vers3, etc below) 2 - need something to represent how they are
 multiplied: a*b,c, a, bc a,b,c, abc, etc 3 - need something to represent how
 many times each group can be applied (exp)

        in the conformal model, versors can be represented by circles and
 spheres, allowing for hyperbolic groups
...
class PointGroup {
 protected:
  // The "Frame" is composed of 2 or 3 versors (could be dual planes ... or also
  // circles or spheres for hyperbolic extension)
  Versor mVersor[3];
  ...
  // Generators made From the Frame (reflectors, glide reflectors, etc) to make
...
inline ostream& operator<<(ostream& os, const PointGroup& g) {
  os << g.name() << " Group:  #Frames: " << g.numFrame()
     << " #Gens: " << g.numGen() << "\n";
...
class PointGroup2D : public PointGroup {
 public:
  /// 2d point groups -> pass in number of exponents (foldness) and boolean spin
  /// or pin type (bar = true = spin, false = mirror)
