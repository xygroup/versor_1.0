...
struct GLVInput : public Interface::InputImpl {
  ...
  virtual void getData(void* udata) {
    ...
    interface->mouse.pos = Vec(glv.mouse().x() / glv.width(),
                               1 - glv.mouse().y() / glv.height(), 0);
    interface->mouse.move = Vec(glv.mouse().dx() / glv.width(),
                                -glv.mouse().dy() / glv.height(), 0);
...
struct GLVApp : public View3D {
  ...
  virtual void gl2ps() {
    ...
    while (state == GL2PS_OVERFLOW) {
      buffsize += 1024 * 1024;
      gl2psBeginPage(
          "test", "gl2psTestSimple", tv, GL2PS_EPS,
          GL2PS_SIMPLE_SORT,  // GL2PS_NO_SORT,//, //GL2PS_SIMPLE_SORT,
          GL2PS_NO_PS3_SHADING | GL2PS_BEST_ROOT | GL2PS_SIMPLE_LINE_OFFSET |
...
//           string ts = Print::TikzPerspective(s, camera());
  ...
  */
  ...
  GLint nr, GLint ng, GLint nb, GLint buffersize, FILE *stream, const char
  *filename )
