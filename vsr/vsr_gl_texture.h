...
class Texture {
 ...
 public:
  ...
  void set(
      GL::PARAM a,
      GL::PARAM b);  ///< Set param named a to param b of this texture target
  ...
  void print() {
    std::cout << GL::Get(mTarget) << GL::Get(mFormat) << GL::Get(mType)
              << "Chan: " << mNumChannels << " datasize: " << dataSize()
              << " numtexels " << numTexels() << std::endl;
  }
...
) : mWidth(w), mHeight(h), mResWidth(wres), mResHeight(hres), mTex(NULL) { mTex
= new Texture(mResWidth, mResHeight);
                        }
