...
struct Pipe {
  ...
  static void InitBufferObjects() {
    ...
    //                        LightPosition.set( Shader :: Current(),
    //                        "lightPosition");
  ...
  static int Buffer(Mesh mesh, int id = -1) {
    ...
    mbo[tmp].vertex = VBO(&mesh.vertices()[0].Pos[0], mesh.num(),
                          mesh.num() * sizeof(Vertex), GL::VERTEXBUFFER);
    mbo[tmp].index = VBO(&mesh.indices()[0], mesh.numIdx(),
                         mesh.numIdx() * sizeof(GLuint), GL::ELEMENTBUFFER);
    return tmp;
  }

  static int Update(int id, Vertex* val) { mbo[id].vertex.update(val); }

  static void Pointer() { vatt.pointer(); }

  static void Enable() { vatt.enable(); }

  static void Disable() { vatt.disable(); }

  static void DrawElements(int id, GLenum mode, int num = -1, int off = 0) {
    mbo[id].index.drawElements(mode, num, off);
  }

  static void DrawArray(int id, GLenum mode) { mbo[id].vertex.drawArray(mode); }
...
void Array(const A&);
template <>
void Array(const Vec&) {}
template <>
void Array(const Drv&);
template <>
void Array(const Biv&);
template <>
void Array(const Pnt&);
template <>
void Array(const Par&);
template <>
void Array(const Cir& c) {
...
// Bind to Shader
template <class A>
void InitBuffer(const A&);
template <class A>
void Buffer(const A&);
template <>
void Buffer(const Vec&) {}
template <>
void Buffer(const Drv&);
template <>
void Buffer(const Biv&);
template <>
void Buffer(const Pnt&);
template <>
void Buffer(const Par&);

template <>
void InitBuffer(const Cir& c) {}
template <>
void Buffer(const Cir& c) {
...
template <class A>
void RenderES(const A& a, ShaderProgram& program) {}
template <class A>
void RenderES(const A* a, int num, const Mat4f& mvm, ShaderProgram& program) {}

template <>
void RenderES(const Cir& cir, ShaderProgram& program) {
...
template <>
void RenderES(const Cir* fcir, int num, const Mat4f& mvm,
